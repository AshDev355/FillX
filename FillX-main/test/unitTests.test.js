...
import { classifyQuestion, generateLocalAnswer } from '../src/shared/answerGenerator.js';
...
test('3a. Matching Heuristics: sibling labels never inherit full name', () => {
  const fields = [
    { fieldId: 'name', label: 'Full Name', type: 'text', tagName: 'INPUT', nearbyText: 'Full Name Email Address Phone Number City Country' },
    { fieldId: 'email', label: 'Email Address', type: 'email', tagName: 'INPUT', nearbyText: 'Full Name Email Address Phone Number City Country' },
    { fieldId: 'phone', label: 'Phone Number', type: 'tel', tagName: 'INPUT', nearbyText: 'Full Name Email Address Phone Number City Country' },
    { fieldId: 'city', label: 'City', type: 'text', tagName: 'INPUT', nearbyText: 'Full Name Email Address Phone Number City Country' },
    { fieldId: 'country', label: 'Country', type: 'select', tagName: 'SELECT', nearbyText: 'Full Name Email Address Phone Number City Country' },
  ];
  ...
  assert.ok(results.every((result) => result.status === MATCH_STATUS.MATCHED));
  assert.equal(results[1].confidence, 0.99);
  assert.equal(results[2].confidence, 0.96);
  assert.equal(results[3].confidence, 0.94);
  assert.equal(results[4].confidence, 0.92);
});
...
test('4a. Contextual answer generation uses question intent', () => {
  assert.equal(classifyQuestion('Describe Your Favorite Hobby'), 'personal-interest');
  const hobby = generateLocalAnswer('Describe Your Favorite Hobby', 'painting, crochete, sketching', MOCK_USER_PROFILE);
  assert.match(hobby, /crochet/);
  assert.doesNotMatch(hobby, /high-impact results|core competencies/);

  const motivation = generateLocalAnswer('Why are you interested in this position?', 'React, collaboration', MOCK_USER_PROFILE);
  assert.match(motivation, /interested|opportunity|experience/i);
  assert.notEqual(hobby, motivation);
});
