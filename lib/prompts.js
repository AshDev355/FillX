...
Extract only the fields found in the text. Use an empty string "" for a missing text field, and an empty array [] for a missing list. Do not invent or guess any missing values.

The JSON MUST have exactly this structure (this matches the app's stored profile shape):
{
  "personal": {
    "firstName": "string",
    "lastName": "string",
...
Classify the question first. For hobbies, interests, and personal questions use a warm, conversational tone. For motivation and experience questions use a concise professional tone. Never use job-application language for a hobby question.
Write a specific first-person response (2-4 sentences) to the question.

Use relevant profile details naturally, correcting obvious spelling artifacts such as "crochete" to "crochet". Do not just list keywords verbatim. Do not include markdown fences or introductory/concluding prose outside the answer itself.
