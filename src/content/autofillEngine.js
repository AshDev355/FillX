 ...
 * - Attention counter updates (fieldsNeedAttention = ambiguous + unmatched)
 * - Save-prompt attachment for unmatched fields (Member 4 integration)
 *
 * Safety Guarantees:
 * - NEVER automatically clicks submit buttons or submits forms.
 * - NEVER invents values for unmatched fields.
 * - Fails gracefully if DOM elements disappear or change.
 */
 ...
 * @returns {object} Execution summary and field statistics
 */
export function executeAutofill(matchingResults, options = {}) {
  ...
  // Clear previous highlights if requested or by default
  ...
  for (const match of matchingResults) {
    const { fieldId, status, value, confidence, isOpenEnded } = match;
    ...
    switch (status) {
      ...
      default: {
        // Unmatched field: leave strictly blank, highlight red/orange
  ...
  // Handle any detected fields that were omitted from matching results (mark as unmatched)
  ...
  // Attach Member 4's save prompt listeners to unmatched fields
