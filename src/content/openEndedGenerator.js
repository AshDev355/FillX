/**
 * openEndedGenerator.js — Keyword-to-Answer Generator for Open-Ended Questions
 *
 * Implements Phase 9 of the Project Plan:
 * 1. Detects open-ended / essay-style form fields (textareas, "why", "describe", "tell us about").
 * 2. Injects a stylish "✨ Generate answer" trigger next to the field.
 * 3. Shows a keyword prompt modal on click.
 * 4. Calls the background service worker (/api/generate route) and injects the AI paragraph into the textarea.
 */
...
/**
 * Removes any open keyword prompt modal.
 */
...
/**
 * Displays the keyword input modal positioned near the target field.
 *
...
function showKeywordModal(element, question, fieldId) {
  ...
  modal.innerHTML = `
    ...
    <p>Enter a few keywords describing what you want to highlight for: "<em>${escapeHtml(question)}</em>"</p>
      ...
      chrome.runtime.sendMessage(
        ...
        (response) => {
          if (response?.success && response.answer) {
            ...
            console.warn('FillX: AI generation response empty or failed:', response?.error);
          }
          removeModal();
        }
      );
    } else {
      // Local offline fallback synthesis for testing
      const fallbackAnswer = `Based on my background in software development and key strengths in ${keywords}, I have consistently delivered robust solutions. For ${question}, my experience demonstrates strong execution and dedication to high-quality results.`;
...
/**
 * Injects "✨ Generate answer" buttons next to open-ended fields.
 *
 * @param {Array<object>} fields - Array of detected field descriptors
 */
...
/**
 * Removes all generate answer buttons from the page.
 */
