 ...
 *
 * Runs end-to-end assertions against the Member 1 autofill engine,
 * verifying detection, identification, framework value setters, safety guards,
 * highlighting, and attention counters.
 */

import { MOCK_USER_PROFILE, MOCK_MATCHING_RESULTS_SAMPLE } from './mockData.js';
...
/**
 * Updates status counters on the test harness.
 */
...
/**
 * Logs a message to the in-page test dashboard.
 */
...
/**
 * Main automated test suite execution.
 */
export async function runAutomatedTests() {
  resultsPanel.style.display = 'block';
  testLogEl.innerHTML = '';
  testSummaryEl.textContent = 'Running FillX Member 1 Test Suite...';
  ...
  logInfo('Starting Step 1: Field Detection Verification');
  ...
  logInfo('Starting Step 2: Unique Field Identification & DOM Lookup');
  ...
  logInfo('Starting Step 3: Framework-Aware Value Setting');
  ...
  assert('React Controlled Input: Dispatched input event triggers listener', reactStateDisplay.textContent.includes('Acme Software Corp'));
  ...
  assert('Vue Reactive Input: Dispatched event triggers listener', vueStateDisplay.textContent.includes('Senior Frontend Developer'));
  ...
  logInfo('Starting Step 4: Autofill Engine Execution & Matching States');

  // Test 5: Autofill Execution with Mock Matching Results
  // Build matching results mapped to current detected fields
  const mockMatching = fields.map((f) => {
    ...
    if (clues.includes('notes') || clues.includes('why us')) {
      return { fieldId: f.fieldId, status: 'ambiguous', value: 'Experienced engineer interested in mission.', confidence: 0.65, isOpenEnded: true };
    }
  ...
  logInfo('Starting Step 5: Highlighting & Reversibility');
...
btnScan.addEventListener('click', () => {
  const engine = window.__FILLX_CONTENT_SCRIPT__;
  if (!engine) return;
  const { fields, stats } = engine.handleScanPage();
  updateHarnessStats(stats);
});
